<template>
  <div class="max-w-4xl mx-auto pt-10 px-6">
    ...
    <!-- Balance Overview -->
    <section
      class="bg-gradient-to-r from-orange-200 to-primary text-white p-6 rounded-xl shadow-lg mb-6"
    >
      <div class="text-center">
        <h2 class="text-lg font-medium mb-2">Available Balance</h2>
        <strong class="text-3xl mb-2"
          >₦{{
            (wallet.balance || "0.00").toLocaleString("en-NG", {
        ...
        <div class="flex justify-between py-2 px-3">
          <p class="text-gray-600">
            Data updated {{ lastUpdate }} second<span v-if="lastUpdate !== 1"
              >s</span
            >
            ago
          </p>
    ...
    <div class="bg-white shadow-md rounded-lg p-6 mb-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div class="flex flex-col items-center justiy-center h-full">
            <UIcon name="i-material-symbols-send" class="w-8 h-8 mb-2" />
        ...
        >
          <div class="flex flex-col items-center justiy-center h-full">
            <UIcon name="i-material-symbols-receipt" class="w-8 h-8 mb-2" />
    ...
    <section
      ref="historySection"
      class="bg-gray-50 font-manrope shadow-md rounded-lg p-6 mb-6"
    >
...
const fliteredTxs = computed(() => {
  ...
  return transactions.value
    .filter((tx) =>
      activeFilter.value === "all" ? true : tx.type === activeFilter.value
    )
    .filter(
      (tx) =>
        !query ||
        tx.id.toLowerCase().includes(query) ||
        tx.titletoLowerCase().includes(query)
    );
});
