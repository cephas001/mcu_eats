  ...
  <div class="max-w-4xl mx-auto pt-2 px-6">
    ...
    <header class="text-center mb-6">
      <div class="flex justify-between items-start">
        <div class="text-left">
          ...
          <div class="flex items-center gap-2">
            ...
            <select 
              ...
              :class="status === 'Available' ? 'text-green-600 border-green-600' : 'text-red-700 border-red-700'"
              >
        ...
        <Media src="/Delivery.jpg" class="w-20 rounded-full"/>
      </div>
    </header>
    ...
    <section class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-5">
      <div class="bg-blue-50 p-4 rounded-lg shadow-md flex justify-between">
        <div>
           ...
           <p class="text-2xl">{{  profile?.data?.activeDeliveries?.length || 0 }}</p>
      ...
      <div class="bg-green-50 p-4 rounded-lg shadow-md flex justify-between">
        <div>
           ...
           <p class="text-xs text-gray-600 mt-1"> From {{  profile?.data?.todayDeliveries || 0 }} deliveries</p>
        </div>
      ...
      <div class="bg-yellow-50 p-4 rounded-lg shadow-md flex justify-between">
        <div>
           ...
           <p class="text-xs text-gray-600 mt-1">{{  profile?.data?.totalReviews || 0 }} reviews</p>
        </div>
      ...
      <div class="bg-orange-50 p-4 rounded-lg shadow-md flex justify-between">
        <div>
           ...
           <p class="text-2xl">{{  profile?.data?.onlineHours || '0h:00m:00s' }}</p>
      ...
      <div class="bg-red-50 p-4 rounded-lg shadow-md flex justify-between">
        <div>
           ...
           <p class="text-2xl">{{  profile?.data?.pendingPickups?.length || 0 }}</p>
        ...
        <h3 class="text-lg font-bold text-gray-900 mb-4">Current Delivery</h3>
          <div v-if="profile?.data?.currentDelivery" class="space-y-3">
            <div class="flex justify-between items-center">
              ...
              <span class="font-medium">#{{ profile?.data?.currentDelivery?.orderId || 432436}}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-gray-600">Customer:</span>
              <span class="font-medium">{{ profile?.data?.currentDelivery?.customerName || 'Okodugha Peter'}}</span>
            </div>
            <div class="flex justify-between items-center">
              <span class="text-gray-600">Address:</span>
              <span class="font-medium">{{ profile?.data?.currentDelivery?.deliveryAddress || 'Room 5 Atuwatse Hostel'}}</span>
            </div>
          </div>
